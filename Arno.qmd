...
movies_genre_extended |>
  ...
  ggplot(aes( x = year, y = avggross)) + # maybe it is better to plot by avg gross instead of only gross
    ...
    facet_grid(~genre) +
    labs(title = "It seems that Action, Adventure, Fantasy & Sci-Fi, hold the best Gross result in modern times",
         subtitle = "On the other hand genres like War, Wester & Sport faced a decline in recent years", 
         x = "Year for each genre (1920-2023)",
         y = "Average gross by year")
...
movies_genre_extended |> 
  ...
  summarise(sum_gross = sum(gross,na.rm = T)) |>
  ggplot(aes( x = genre , y = sum_gross)) + 
  geom_point(aes(color = certificate)) + geom_boxplot()
