...
return_selected_genres <- function(selected_genres, collapsed_genres){
  movie_genres <- unlist(strsplit(collapsed_genres, ","))
  browser()
  result <- movie_genres[movie_genres %in% selected_genres]
  if(length(result)==length(selected_genres)){
...
filtered_movies <- reactive({
  ...
  if (length(selected_genres) == 0) { #or if("All" %in% selected_genres) but we want none!
    ...
    filtered_data <- filter(movies |>
        ...
        ungroup()|>
        unique() |>
        mutate(fits_genres = return_selected_genres(selected_genres,genre)), 
      fits_genres %in% genre)# genre %in% selected_genres)
  }
