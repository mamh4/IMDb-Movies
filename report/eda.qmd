...
We managed to scrape approx. 60% of the date of births of directors (4347 total number of directors) and approx. 66% of the starring cast. The reason as to not getting full information can be attributed to either:
...
movies |> distinct(year, gross,gross_CPI_adjusted, .keep_all = TRUE) |>
  ...
  geom_line(aes(y=total_gross_CPI_adjusted),col = "purple",data = .%>%
  ...
  labs(title = "Total Gross and CPI Udjusted Trend Upwards",
       subtitle = "A noticable decline around Covid-19 (2020-2022) and a surprising behaviour\naround WWII (1935-1945)",
...
movies |> distinct(year, gross,gross_CPI_adjusted, .keep_all = TRUE) |>
  ...
  annotate("text", x = 1935, y = 45e8, vjust = "bottom", hjust = "left",
           label = "Gross", color = "red")+
  labs(title = "The Surprising Behaviour is During The Years 1937, 1940 and 1942",
       y = "Total Gross\nTotal CPI Adjusted Gross")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust=1))
...
movies |> distinct(title,year,gross_CPI_adjusted,gross) |>
  ...
  labs(title = "Snow White and the Seven Dwarfs Comprises More than 97%\nof the Gross Amount of The Year 1937",
       x="Movie Titles With Gross Amount",
...
movies |> distinct(title,year,gross_CPI_adjusted,gross) |>
  ...
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = scales::comma, limits = c(0,45e8))+
  labs(title = "Pinocchio (48.4%) and Fantasia (43.9%) Together Comprise\n92% of The Gross Amount of The Year 1940",
       x="Movie Titles With Gross Amount",
       y="CPI Adjusted Gross Amount")+
  theme_minimal() +
  coord_flip()
...
movies |> distinct(title,year,gross_CPI_adjusted,gross) |>
  ...
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = scales::comma, limits = c(0,45e8))+
  labs(title = "Bambi Comprises Almost 73% of the\nGross Amount of The Year 1942",
       x="Movie Titles With Gross Amount",
       y="CPI Adjusted Gross Amount")+
  theme_minimal() +
  coord_flip()
...
```{r ratings-analysis}
movies_collapsed |> group_by(year) |>
...
movies |> distinct(title, year,run_time, .keep_all = TRUE) |>
  ...
  labs(y="Estimated Total Tickets Sold", 
       ...
       subtitle = "Perturbations during WWII (1936-1945) analysed earlier can be attributed to the titles\nSnow White and the Seven Dwarfs (1937), Pinocchio and Fantasia (1940), Bambi(1942).\nDrop around Covid-19 (2020-2022)")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust=1))
```
This plot shows a very similar pattern to CPI Adjusted Gross we've witnessed earlier. Similarly the cause for the surprising peaks between 1935 and 1945 is attributed to the movies Snow White and the Seven Dwarfs (1937), Pinocchio and Fantasia (1940), Bambi(1942).
...
movies |> distinct(title,year,run_time,population, .keep_all = TRUE) |>
  ...
  labs(y="Proportion of Estimated Views",
       subtitle = "Estimated Percentage of US Population Watching Snow White\nand The Sweven Dwarfs Exceed 80%")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust=1))
