...
```
As expected, older movies tend to do better in terms of gross when we adjust for the consumer price index to today's prices. Highlighted in green however we see a somewhat bizarre behavior. We take a closer look!
...
movies |> distinct(year, gross,gross_CPI_adjusted, .keep_all = TRUE) |>
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))
```
After a closer look we see that those spikes in CPI adjusted gross were in the years 1937, 1939, 1940 and 1942. Let's take a closer look at what happened these years.
...
movies |> distinct(title,year,gross_CPI_adjusted,gross) |>
  ...
  geom_bar(stat = "identity") +
  scale_y_continuous(limits = c(0, 45e8),
